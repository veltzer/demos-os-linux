...
#include <linux/gfp.h> /* for get_markveltzered_page */
#include <linux/cdev.h> /* for cdev_* */
#include <linux/sched.h> /* for cond_resched */

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Mark Veltzer");
MODULE_DESCRIPTION("A simple implementation for something like /dev/markveltzer");

/*
 * This is an implementation of a kernel module which exposes a /dev/markveltzer like
 * device to user space. Whenever someone reads from the device the buffer passed
 * to kernel gets filled with markveltzers.
...
const char *my_buf = "markveltzer";

/* these are the actual operations */

static ssize_t read_markveltzer(struct file *file, char __user *buf, size_t count,
		loff_t *ppos)
{
	if (count < 11)
		return -EFAULT;
	if (copy_to_user(buf, my_buf, 11))
		return -EFAULT;
	*ppos += 11;
	return 11;
}
...
static const struct file_operations markveltzer_fops = {
	.owner = THIS_MODULE,
	.read = read_markveltzer,
};
...
static int markveltzer_init(void)
{
	int err = 0;
	...
	cdev_init(&my_cdev, &markveltzer_fops);
...
static void markveltzer_exit(void)
{
	device_destroy(my_class, first_dev);
	class_destroy(my_class);
	cdev_del(&my_cdev);
...
module_init(markveltzer_init);
module_exit(markveltzer_exit);
