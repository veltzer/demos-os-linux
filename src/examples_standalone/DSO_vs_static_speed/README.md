...
b) Potential cache misses: If the GOT entry is not in the CPU cache, there's an additional delay to fetch it from memory.
c) Potential page faults: If the DSO is not loaded into memory, the first call can incur a significant delay due to page faults.
